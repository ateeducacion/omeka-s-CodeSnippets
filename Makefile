...
# Generate the CodeSnippets-X.X.X.zip package
package:
	@if [ -z "$(VERSION)" ]; then \
	...
	@echo "Creating ZIP archive: CodeSnippets-$(VERSION).zip..."
	composer archive --format=zip --file="CodeSnippets-$(VERSION)-raw"
	@echo "Repacking into proper structure..."
	mkdir -p tmpzip/CodeSnippets && unzip -q CodeSnippets-$(VERSION)-raw.zip -d tmpzip/CodeSnippets && \
	cd tmpzip && zip -qr ../CodeSnippets-$(VERSION).zip CodeSnippets && cd .. && rm -rf tmpzip CodeSnippets-$(VERSION)-raw.zip
...
enable-module:
	@echo "Enabling CodeSnippets module inside Omeka S..."
	docker compose exec omekas sh -lc 'omeka-s-cli module:install CodeSnippets || true'
